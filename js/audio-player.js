  ...
  function updateMetadata(track) {
    ...
    function nextTrack() {
      if (repeatMode === 2) { // Repeat One
          ...
          // Continuous Play: Loop to start if at end
      ...
      fetch("/tracks.json")
        ...
        .then(data => {
          ...
          sortedReleases.forEach((release) => {
              ...
              // --- UPDATED LOGIC: Conditional Catalog Number ---
              // Checks if catalog exists AND is not just whitespace
              const hasCatalog = release.catalog && String(release.catalog).trim().length > 0;
              
              let metaHtml = '';
              if (hasCatalog) {
                  // Show: Artist • Catalog • Date
                  metaHtml = `${displayArtist(release.artist)} <span class="ap-dot">•</span> ${release.catalog} <span class="ap-dot">•</span> ${release.release_date}`;
              } else {
                  // Show: Artist • Date (No catalog)
                  metaHtml = `${displayArtist(release.artist)} <span class="ap-dot">•</span> ${release.release_date}`;
              }
